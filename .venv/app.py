...
st.title("Wind Resource Assessment Tool")
...
with st.sidebar:
    ...
    start_year = st.number_input("Start year", value=2019, min_value=1980, max_value=2026, step=1)
    end_year = st.number_input("End year", value=2023, min_value=1980, max_value=2026, step=1)
    ...
    if not turbine_names:
        ...
        st.stop()
    selected_turbine = st.selectbox("Select Turbine Model", turbine_names)


# --- Main page: interactive map for site selection ---
st.subheader("Select Site Location")
st.caption("Click anywhere on the map to drop a pin at your site.")
...
if st.session_state["selected_coords"] is not None:
    ...
    folium.Marker(
        [lat_sel, lon_sel],
        tooltip=f"Selected site ({lat_sel:.4f}, {lon_sel:.4f})",
...
if coords is None:
    st.info("No site selected yet. Click the map to choose a location.")
else:
    st.success(f"Selected site: **Lat {coords[0]:.4f}, Lon {coords[1]:.4f}**")

run = st.button("Run Assessment", type="primary", disabled=(coords is None))
...
def weibull_histogram(wind: pd.Series, k: float, c: float) -> go.Figure:
    ...
    fig.add_trace(go.Histogram(
        ...
        name="Observed", marker_color="#4C9AFF", opacity=0.75,
    ))
    x = np.linspace(0, wind.max() * 1.05, 200)
    pdf = weibull_min.pdf(x, k, loc=0, scale=c)
    fig.add_trace(go.Scatter(
        x=x, y=pdf, mode="lines", name=f"Weibull (k={k:.2f}, c={c:.2f})",
        line=dict(color="#FF6B6B", width=3),
    ))
    fig.update_layout(
        title="Wind Speed Distribution vs Fitted Weibull",
        xaxis_title="Wind Speed (m/s)", yaxis_title="Probability Density", bargap=0.02,
    )
    return fig
...
def diurnal_profile_chart(df: pd.DataFrame, col: str) -> go.Figure:
    ...
    fig.update_layout(
        title="Diurnal Wind Profile",
        xaxis_title="Hour of Day (local)", yaxis_title="Mean Wind Speed (m/s)",
...
if "results" in st.session_state:
    ...
    st.subheader(f"Results for ({r['lat']:.4f}, {r['lon']:.4f}) — {r['turbine']}")
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Mean Wind Speed", f"{r['mean_ws']:.2f} m/s")
    m2.metric("Weibull shape (k)", f"{r['k']:.2f}")
    m3.metric("Weibull scale (c)", f"{r['c']:.2f} m/s")
    m4.metric(f"Gross AEP ({r['rated_kw']/1000:.1f} MW)",
              f"{r['aep_mwh']:,.0f} MWh", f"CF {cf:.1f}%")
    ...
    st.divider()
    st.subheader("Save This Project")
